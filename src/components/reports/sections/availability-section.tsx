...
export function AvailabilitySection({
  ...
  return (
    <div className={cn("space-y-6", className)}>
      <h3 className="font-semibold text-lg">Verfügbarkeit</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-muted/50 rounded-lg p-4">
          <p className="text-xs text-muted-foreground mb-1">SLA-Verletzungen</p>
          <p className={cn(
            ...
            {viewMode === "weekly" &&
              data.weeklyBreakdown.map((week) => (
                <TableRow key={week.weekNumber}>
                  <TableCell className="font-mono text-sm">
                    KW {week.weekNumber} ({week.startDate} - {week.endDate})
                  </TableCell>
      ...
      {data.slaBreaches.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-sm font-medium text-muted-foreground">SLA-Verletzungen</h4>
